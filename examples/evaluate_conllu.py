...
def stat_one_tree(lines) :
    ...
    for line in lines :
        ...
        sdp_vals = [val.split(':') for val in payload[8].split('|')]
        head_vals = [val[0] for val in sdp_vals]
        deprel_vals = [val[1] for val in sdp_vals]
        #head_val = payload[6]
...
if __name__ == "__main__" : 
    ...
    elif opts.language == 'udpos':
        engine = lambda x, y: y in set(['PUNCT'])
    else:
    ...
    #print ("NLP:{0[0]:^10},NLR:{0[1]:^10},NUP:{0[2]:^10},NUR:{0[3]:^10}".format(list(map(lambda x : "{:.2f}%".format(x*100) ,[NLP, NLR, NUP, NUR]))))
